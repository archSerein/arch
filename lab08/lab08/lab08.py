...
def add_links(link1, link2):
    ...
    "*** YOUR CODE HERE ***"
    def deep_copy(link):
        """Creates a deep copy of a link."""
        if link is Link.empty:
            return Link.empty
        elif isinstance(link.first, Link):
            return Link(deep_copy(link.first), deep_copy(link.rest))
        else:
            return Link(link.first, deep_copy(link.rest))

    if link1 is Link.empty:
        return deep_copy(link2)
    else:
