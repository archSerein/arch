...
class Game:
    ...
    def play(self):
        while not self.game_over():
            ...
            opponent = self.p2 if self.turn % 2 == 0 else self.p1
            action = current_player.choose(opponent)
            if action == current_player.debate:
                current_player.debate(opponent)
            elif action == current_player.speech:
                current_player.speech(opponent)
            self.turn += 1
        return self.winner()
...
def add_d_leaves(t, v):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(t, v, depths):
        for b in t.branches:
            helper(b, v, depths + 1)
        t.branches.extend([Tree(v) for _ in range(depths)])
    helper(t, v, 0)
...
def deep_map_mut(func, lnk):
    ...
    def helper(func, lnk):
        if lnk is Link.empty:
            return
        elif type(lnk.first) is Link:
            helper(func, lnk.first)
        else:
            lnk.first = func(lnk.first)
        helper(func, lnk.rest)
    helper(func, lnk)
